<template>
    <div class="w-full">
        <p class="md:hidden mt-2 mb-3 font-bold tracking-wider uppercase text-xs">Share this article</p>
...
export default {
    ...
    data: function() {
        return {
            uri: location.href,
            encodedUri: encodeURIComponent(location.href),
            facebookUri: `https://www.facebook.com/sharer/sharer.php?u=${this.encodedUri}`,
            twitterUri: `https://twitter.com/share?url=www.google.com`
        }
    },
    methods: {
        share(uri, type) {
            window.open(`https://twitter.com/share?${this.uri}`, type, "toolbar=no,location=0,status=no,menubar=no,scrollbars=yes,width=550,height=285,resizable=1");
        },
        copyLink() {
            navigator.clipboard.writeText(this.uri);
            this.$emit('showSnackbar', 'Copied URL to clipboard')
        }
    }
}
</script>
